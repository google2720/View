...
public class ChannelManage {
	...
	static {
		...
		OkHttpUtils.get().url("http://www.xiaozhangshi.com/v3.php?ctl=video_keys&action=get_video_keys").build().execute(new StringCallback() {
			@Override
			...
			public void onResponse(String response) {
				ArrayList<Top> tops = parseJson(response);
//				for (int i=0;i<tops.size();i++){
//					defaultUserChannels.add(new ChannelItem(i, tops.get(i).kName, i, 1));
//				}
	 ...
	 * @param json
	 */
	private static ArrayList<Top> parseJson(String json) {
		try {
			ArrayList<Top> topList = new ArrayList<Top>();
			JSONArray array = new JSONArray(json);
			for (int i = 0; i < array.length(); i++) {
				JSONObject jsonObject = array.getJSONObject(i);
				int kid = jsonObject.getInt("kId");
				String kName = jsonObject.getString("kName");
				int keys_Isshow = jsonObject.getInt("keys_Isshow");
				Top top = new Top(kid, kName, keys_Isshow);
				topList.add(top);
			}
			return topList;
