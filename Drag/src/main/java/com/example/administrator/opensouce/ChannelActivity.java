...
public class ChannelActivity extends BaseActivity implements OnItemClickListener {
	...
	private void initView() {
		...
		userGridView.setActivity(this);
		my_category_tip_text =(TextView)findViewById(R.id.my_category_tip_text);
		tv_make =(TextView)findViewById(R.id.tv_make);

	}
     public void setStatuTrue(){
		 tv_make.setText("完成");
		 my_category_tip_text.setVisibility(View.VISIBLE);
		 userAdapter.setDeleteShow(true);
		 userAdapter.notifyDataSetInvalidated();
		 isMake=true;
	 }
	public void setStatucFalse(){
		tv_make.setText("编辑");
		my_category_tip_text.setVisibility(View.GONE);
		userAdapter.setDeleteShow(false);
		userAdapter.notifyDataSetInvalidated();
		isMake=false;
	}
	...
	public void onItemClick(AdapterView<?> parent, final View view, final int position,long id) {
		...
		switch (parent.getId()) {
			case R.id.userGridView:
				if (!isMake){
					saveChannel();
					Intent position1 = getIntent().putExtra("position", position+"");
					setResult(RESULT_OK,position1);
					finish();
				}else {
